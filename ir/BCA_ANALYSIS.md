# BCA（2025-02-28 署名版・全87頁）の分析 ─ Nasdaq宛レター送付前の必読事項

## 0. 結論

**確認済みの事実：Sponsor 側のロックアップ契約は締結されていない。**

したがって Nasdaq 宛レター第7項(b)の記述（"None of those shares was subject to a lock-up
undertaking"）は**事実として正しい**。ただし**不完全**であり、そのままでは当社に不利に働く。

理由：BCA は Recital J および §7.2(j) で **Sponsor のロックアップ契約締結を要求し**、
§7.2(k) で **Sponsor Shares と Escrow Shares のエスクロー預託を要求している**。
「ロックアップが無かった」とだけ書くと、**それが当初からの合意であったかのように読める**。
実際には、**BCA が要求した条件が充足されていない**という事実である。

**→ 第7項(b)に BCA の条文を一文加えるだけでよい。送付を遅らせる必要はない（第5節の修正案）。**

### 最重要：ロックアップが無いことは、売却を正当化しない

| | 性質 | 効果 |
|---|---|---|
| ロックアップ | **契約** | 無ければ契約上の制限は無い |
| **Rule 144(i)** | **法律** | **契約の有無と無関係に適用される** |
| Schedule 13D | **法律** | **契約の有無と無関係に適用される** |

BIOT は元シェル会社（SPAC）の承継会社であり、Rule 144 は Form 10 相当情報の提出から
12か月を経過するまで利用できない（＝2027年7月頃まで）。**ロックアップが無くても、有効な
再販売登録届出書が無ければ、そもそも適法に売却できない。**

**ロックアップの不在は、むしろ「なぜ売れたのか」という問いを鋭くする。**

---

## 1. BCA の該当条文（署名版・全87頁より）

### 定義（Article XI, p.81-82）
| 用語 | 定義 |
|---|---|
| **"Sponsor"** | **Relativity Acquisition Sponsor, LLC**, a Delaware limited liability company ─ **単数・1社のみ** |
| **"Sponsor Shares"** | Sponsor が Closing 前の private placement で取得した Purchaser Common Stock **全部** |
| Purchaser Representative | Relativity Acquisition Sponsor, LLC（同一法人が両方の地位を兼ねる） |

### Recital I（p.3）─ Sponsor Escrow Agreement
> the Sponsor shall deposit **1,000,000 Pubco Common Stock**... (**"Escrow Shares"**) into escrow
> **upon the Closing for a period of up to six (6) months following the Closing**

→ Closing は 2026-07-24。**2027年1月頃まで 1,000,000 株はエスクローにあるはず。**

### Recital J（p.3）─ Lock-Up Agreement
> the Seller **and the Sponsor** will have **each** entered into a Lock-Up Agreement with Pubco,
> the Purchaser and the Purchaser Representative... each of which Lock-Up Agreement shall
> **become effective as of the Effective Time**

→ **Sponsor もロックアップを締結する建て付け。**「創業者側だけ」ではない。

### §7.2(j)（p.61）─ クロージング条件
> **Certain Ancillary Documents.** The **Lock-Up Agreements**... shall be **in full force and
> effect** in accordance with the terms thereof **as of the Closing**.

### §7.2(k)（p.61）─ クロージング条件【最重要】
> **Sponsor Escrow Agreement.** The Sponsor shall have duly executed and delivered the Sponsor
> Escrow Agreement and shall have **deposited the Sponsor Shares AND Escrow Shares into escrow**
> in accordance with the terms and subject to the conditions set forth therein.
> **Evidence of such deposit shall have been provided to the Target Company to its reasonable
> satisfaction.**

→ エスクローに入れるべきは 1,000,000 株だけではなく、**Sponsor Shares（＝Sponsor が保有する
私募株式の全部）＋ Escrow Shares** の両方。
→ §7.2 は **Target Company（当社側）のために設けられたクロージング条件**であり、
   **預託の証拠を当社に提出する義務**まで明記されている。

---

## 2. 事実との突合

| BCA の要求 | 実際（Continental の記録） |
|---|---|
| Sponsor Shares + Escrow Shares をエスクローに預託（§7.2(k)） | **2026-07-24 の登録株主名簿にエスクロー口座は存在しない**（該当は DWAC Control Account 15,279株のみ） |
| Sponsor のロックアップが Closing 時点で有効（§7.2(j)・Recital J） | **Sponsor 側のロックアップ契約は締結されていない**（確認済み）。上場当日 2026-07-24 に 2,855,381 株の制限が解除されている |
| Sponsor = 1社（Relativity Acquisition Sponsor, LLC） | 名簿上、同系列とみられる**6社**に分散されている |

**確認済み：Sponsor 側のロックアップ契約は存在しない。**

したがって残る説明は次の2つに絞られる。

1. §7.2(j)(k) のクロージング条件が**放棄（waive）された**
2. 条件が満たされないままクロージングされた

**→ どちらであれ、次が最大の論点になる。**

> **§7.2 は当社（Target Company）の利益のために置かれた条件である。**
> **放棄できるのは当社側だけである。誰が、どの権限で放棄したのか。**

1 であれば放棄書が存在するはずであり、その署名者を確認すること。
2 であれば、Sponsor は BCA 上の義務を履行しないままクロージングしたことになる。

**社内で先に確定させること。外部に出す前に、ここだけは。**

---

## 3. Schedule 13D ─ 6社への分散は免責理由にならない

### 3-1. 分散という行為そのものが報告事由
20-F Item 7.A は Closing 時点で Relativity Acquisition Sponsor, LLC を 5,515,481株
（19.15%）と報告している。その株式を他の5社へ移した行為は、**それ自体が 1% を超える
"disposition"** であり、Schedule 13D が提出済みなら 13D/A の提出事由となる。
**報告義務の対象物を手放すことで報告義務を消すことはできない。**

### 3-2. Section 13(d)(3) ─ グループ規制
共同して取得・保有・議決・処分を行う2以上の者は「グループ」として**単一の person** と
みなされ、合算で判定される。6社合計 19.14% は明確に 5% 超。
共通支配・共通の管理者・クロージング時に単一の Sponsor 枠から一括配分された事実は、
グループ認定の有力な証拠となる。

### 3-3. 5%直下への張り付き
発行済 28,794,472株（2026-08-05 の 15,279株消却後）に対し：

| エンティティ | 株数 | 比率 |
|---|---:|---:|
| Relativity II Acquisition Sponsor LLC | 1,435,000 | **4.984%** |
| MGMTT LLC | 1,292,749 | 4.490% |
| BRACS Capital Sponsor LLC | 1,003,175 | 3.484% |
| Paramount Merger Corp Sponsor LLC | 1,003,175 | 3.484% |
| Relativity Acquisition Sponsor LLC | 631,382 | 2.193% |
| Hithos II LLC（20-F算入分） | 150,000 | 0.521% |

- **Relativity II は 5% まであと 4,724 株。** 偶然とは考えにくい。
- **BRACS と Paramount は 1,003,175 株で完全に同額。** 機械的な分割の痕跡。

### 3-4. Section 16 は適用外
BIOT は Form 20-F 提出会社＝外国民間発行体。Rule 3a12-3(b) により Section 16 は適用外で
Form 4 は存在しない。Section 13(d) は適用される。

---

## 3-5. 「Relativity Sponsor は実際には数十社の総称だった」との情報について

### BCA の文言上は "Sponsor" は単数・1社のみ
全87頁を検索した結果：

- **"Sponsors"（複数形）は一度も出現しない。**
- **"Sponsor" = Relativity Acquisition Sponsor, LLC, a Delaware limited liability company**
  （Article XI 定義, p.81）の1社のみ。

**→ 当社が「1社」と理解していたのは誤解ではない。契約書がそう書いてある。**

### 開示されるべきだった場所 ── Schedule 3.5(a) / 3.5(c)

§3.5(a)：
> The issued and outstanding Purchaser Securities as of the date of this Agreement are
> **set forth on Schedule 3.5(a)**.

§3.5(b)：
> **Except as set forth on Schedule 3.5(a) or Schedule 3.5(c)**, there are no ... (iii)
> subscriptions or other rights, **agreements, arrangements, Contracts or commitments of any
> character** ... (A) **relating to the issued or unissued shares or any other equity
> securities of Purchaser**

§3.5(c)：
> **Except as set forth on Schedule 3.5(c)**, there are no **shareholders' agreements, voting
> trusts or other agreements or understandings** to which Purchaser is a party or of which it
> has Knowledge **with respect to the voting of any shares of the Purchaser**.

**→ 数十社の関与を定める取決めが存在したのであれば、Schedule 3.5(a) または 3.5(c) に
記載されていなければならない。記載がなければ §3.5(b)(iii) の表明保証違反。**

**⚠️ 署名版87頁の PDF に Schedule 3.5(a) および 3.5(c) は添付されていない（Annex I のみ）。
最優先で取り寄せること。**

### 法的効果 ── 過度に頼るべき論点と、そうでない論点

| 論点 | 数十社という事実の影響 | 依存度 |
|---|---|---|
| Section 13(d)(3) グループ認定 | **有利になる**。ただし「共同して行動する合意」の立証が必要 | **頼りすぎないこと** |
| **Rule 144(i)** | **一切影響なし。1社でも数十社でも全員が使えない** | **本命。ここが崩れない** |
| Sponsor 自身による 19.15% の処分 | **一切影響なし。受領者が誰であれ処分は処分** | **崩れない** |
| BCA §3.5 表明保証違反 | **有利になる**（Schedule に記載がない場合） | 中 |

**結論：グループ論は補強材料に留め、Rule 144(i) と「処分そのものが報告事由」の2点を軸に
据えること。この2つは相手方の構成がどうであれ揺るがない。**

### 確定 ── 全部で6法人

会長確認済み：**スポンサー側は全部で6法人。個人の社員ではなく、すべて法人。**

これは 2026-07-24 の登録株主名簿と完全に一致する。

| # | 法人 | 7/24 名簿 |
|---|---|---:|
| 1 | Relativity Acquisition Sponsor LLC | 631,382 |
| 2 | Relativity II Acquisition Sponsor LLC | 1,435,000 |
| 3 | MGMTT LLC | 1,292,749 |
| 4 | BRACS Capital Sponsor LLC | 1,003,175 |
| 5 | Paramount Merger Corp Sponsor LLC | 1,003,175 |
| 6 | Hithos II LLC（20-F算入分 150,000） | 250,000 |
| | **合計（20-F算入ベース）** | **5,515,481** |

**→ Form 20-F Item 7.A が「Relativity Acquisition Sponsor, LLC ─ 5,515,481株（19.15%）」
   として報告している数字と、1株単位で一致する。**

---

## 3-6. グループ認定の決定的証拠 ── Transfer Journal 2070304 / 2070305

**Continental Stock Transfer の移動明細に、次の2件が並んでいる。**

| SEQ | Receipt# | 効力発生日 | 譲渡人 | 数量 | DTC Control No. | Broker |
|---:|---:|---|---|---:|---|---|
| 146 | **2070304** | 2026-07-29 | MGMTT LLC | 1,289,000 | **P202620906268** | **PERSHING #443** |
| 147 | **2070305** | 2026-07-29 | Paramount Merger Corp Sponsor LLC | 1,003,175 | **P202620906267** | **PERSHING #443** |

**独立した第三者同士であれば、こうはならない。**

1. **同一日**（2026-07-29）
2. **同一ブローカー**（Pershing、DTC参加者番号 #443）
3. **Receipt番号が連番**（2070304 → 2070305）
4. **DTC Control番号も連番**（...6268 / ...6267）

→ **同一の窓口で、連続して、まとめて処理された**ことを意味する。
   Section 13(d)(3) の「共同して処分を行う合意」の存在を推認させる、極めて強い間接事実。

さらに前日 2026-07-28、BRACS Capital Sponsor LLC が全量 1,003,175 株を DTC 移管しており、
Continental の記録上 **NON-ROUTINE** と分類されている。

**3社が3営業日以内（7/28・7/29・7/29）に、それぞれ全量を DTC へ移している。**

### 補強事実

- **BRACS と Paramount は 1,003,175 株で完全に同額**（機械的な分割の痕跡）
- **Relativity II は 5% まであと 4,724 株**（1,435,000 / 28,794,472 = 4.984%）
- **6社の合計が、20-F が単一名義で報告する数字と1株単位で一致する**

---

## 3-7. ⚠️ 当社側の是正事項 ── Form 20-F Item 7.A の記載

Form 20-F は**当社の提出書類**である。その Item 7.A が
「Relativity Acquisition Sponsor, LLC ─ 5,515,481株」と記載しているが、
**同社が実際に名簿上保有していたのは 631,382 株のみ**であり、残りは他の5法人が保有していた。

**→ 当社自身の開示が不正確である。他者に指摘される前に是正を検討すること。**

**同時に、これは有力な証拠でもある：**
6法人分を単一名義に合算した数字を、**誰かが当社に提供している**。
その提供元がスポンサー側であれば、**スポンサー側自身が6社を一体として扱っていた**ことの
証跡になる。

**取り寄せるべきもの：Item 7.A の数値の出所となった資料（クロージング時の資本構成表、
スポンサー側からの通知、作成時のやり取り）。**

---

## 4. F-4 の開示

BCA も F-4 も "Sponsor" を単数の Relativity Acquisition Sponsor, LLC として記載している。
Closing 時点で実際には6社へ配分されていたのであれば、F-4 の実質株主表の完全性が問題となる。

**⚠️ これは諸刃である。F-4 は当社が署名した登録届出書でもある。**
当社が配分を知らされていなかったのであれば、相手方に対する表明保証違反・補償請求の根拠に
なる一方、当社自身の Section 11 上の立場も検討を要する。**この順序を誤らないこと。**

---

## 5. Nasdaq 宛レターの修正案（第7項(b)）

既存の文は残し、**BCA の条文を述べる一文を挿入するのみ**。断定も非難も加えない。
（**太字部分が追加箇所。**）

> (b) **Deposits into DTC by sponsor entities and by the Company's placement agent.**
> Between 28 and 30 July 2026, 5,060,735 shares were transferred from direct registration
> into DTC by Chardan Capital Markets LLC (1,615,385, its entire holding), BRACS Capital
> Sponsor LLC (1,003,175, its entire holding), Paramount Merger Corp Sponsor LLC (1,003,175,
> its entire holding), MGMTT LLC (1,289,000) and Hithos II LLC (150,000). No lock-up agreement
> was executed by any sponsor entity; the lock-up given at closing binds the founder side of
> the transaction. **The Company notes, so that the Department has the position in full, that
> the Business Combination Agreement dated 28 February 2025 provided at Recital J for the
> Sponsor, defined in that agreement as Relativity Acquisition Sponsor, LLC, to enter into a
> lock-up agreement effective at the Effective Time, and at Section 7.2(k) for the deposit of
> the Sponsor Shares and the Escrow Shares into escrow at Closing. The Company is establishing
> the closing record and makes no complaint at this stage.** A deposit into DTC is the step
> that precedes a sale through a brokerage account. This is the second reason the question in
> section 5 is confined to the two sessions that precede those deposits.

**効果：**
- 事実を隠さない（レター全体の姿勢と整合する）
- 「ロックアップが無いのは当初からの合意」という誤読を防ぐ
- 違反を主張しないので、当社の立証責任が生じない
- **送付を遅らせる必要がない。** 放棄書の有無が判明する前でも成立する書き方である

**判断が必要な点：** 本件は Listing Qualifications 部門（Wayne Bush 氏）にも写しが送られる。
クロージング条件の不充足は、理論上は de-SPAC の成立過程に関する論点にもなりうる。
それでも記載を推奨する理由は、**Nasdaq は同じ記録を必ず入手するため、当社が先に出した方が
一貫性を保てる**こと。ただし最終判断は米国証券弁護士に委ねること。

---

## 6. 直ちに取り寄せるべき書類

1. **Lock-Up Agreement 本体**（Seller 分・Sponsor 分の双方）
2. **Sponsor Escrow Agreement 本体**、およびエスクロー預託の証拠（§7.2(k) により当社が
   受領しているはずのもの）
3. **クロージング証書一式**（closing certificates, secretary's certificates, waiver letters）
   → §7.2(j)(k) の放棄書が存在するか、存在するなら誰の署名か
4. **F-4 の実質株主表**と、Closing 時点の実際の配分内訳
5. **7/24 の制限解除に関する Continental への指示書と弁護士意見書**
